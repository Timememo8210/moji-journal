...
export default function CalendarView({ entries, onSelectDate, onClose }: CalendarViewProps) {
    ...
    >
      <div className="max-w-journal mx-auto px-6 py-4">
        ...
        <div className="flex items-center justify-between mb-4">
          <button
            onClick={prevMonth}
            className="w-11 h-11 rounded-full flex items-center justify-center text-gray-400 hover:text-gray-900 dark:hover:text-white hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors"
          >
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              onClick={goToday}
              className="text-xs text-gray-400 hover:text-gray-700 dark:hover:text-gray-300 px-2 py-0.5 rounded-full border border-gray-200 dark:border-gray-700 hover:border-gray-300 dark:hover:border-gray-600 transition-colors"
            >
              {t('calendarToday')}
            </button>
          </div>
                ...
                {hasEntries && (
                  <span
                    className={`absolute bottom-1 w-1.5 h-1.5 rounded-full ${
                      isToday ? 'bg-white dark:bg-gray-900' : 'bg-amber-500 dark:bg-amber-400'
                    }`}
                  />
                )}
              </button>
